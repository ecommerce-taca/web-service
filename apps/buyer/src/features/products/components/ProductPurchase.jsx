    ...
    <div className="bg-white p-6 border border-border flex flex-col gap-4">
      ...
      {product.vouchers && product.vouchers.length > 0 && (
        <div className="flex items-center justify-between border border-border p-3">
          <div className="flex items-center gap-2">
            <div className="bg-sale text-white text-[10px] font-bold px-2 py-1">VOUCHER</div>
            <span className="text-[14px] text-main font-medium">{product.vouchers[0].offer}</span>
          </div>
          <button className="text-primary text-[14px] font-bold">Xem voucher</button>
        </div>
      )}
      ...
      {product.variants && product.variants.map((v, i) => (
        <div key={i} className="flex flex-col gap-2">
          <span className="text-[14px] font-bold text-main uppercase">
              ...
              return (
                <button 
                  ...
                  className={`px-4 py-2 text-[14px] transition-colors rounded-[8px] border ${
                    isActive 
                      ? 'bg-indigo-50 border-primary text-primary font-bold' 
                      : 'bg-white border-border text-main hover:border-primary'
                  }`}
                >
                  {opt}
                </button>
      ...
      <div className="flex gap-4 mt-4">
        ...
        <Button 
          variant="outline" 
          className="flex-1 h-[48px] text-[16px] rounded-[8px] !border-border !text-main hover:!border-primary hover:!text-primary"
        >
          Thêm vào giỏ
        </Button>
